...
df = spark.createDataFrame([
    Row(primocampo=1, secondocampo=2, terzocampo='felice'),
    Row(primocampo=1, secondocampo=2, terzocampo='triste'),
    Row(primocampo=1, secondocampo=2, terzocampo='felice')
])
...
#df.printSchema()
#df.show()
df=df.selectExpr("CAST(primocampo as STRING)") \
    .select(from_json("primocampo",schema=schema).alias("data"))\
    .select("data.*")
